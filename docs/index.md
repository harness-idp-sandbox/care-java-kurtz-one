...
# run app
java -jar target/java-kurtz-one-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/java-kurtz-one:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/java-kurtz-one -n care

# get external endpoint
kubectl get svc java-kurtz-one-svc -n care
```
...
```bash
kubectl port-forward svc/java-kurtz-one-svc 8080:80 -n care
...
kubectl rollout restart deploy/java-kurtz-one -n care
kubectl rollout status deploy/java-kurtz-one -n care
```
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/javakurtzone/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
...
© ian.kurtz@harness.io. For demo/PoV purposes.
